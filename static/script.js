...
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// Generate Timetable (NOT saving to DB, just for preview)
// ---------------------------------------------------------------
...
// ---------------------------------------------------------------
// Regenerate ONLY current section (actually all, but saves versions)
// ---------------------------------------------------------------
// ---------------------------------------------------------------
// Regenerate & SAVE timetable (all sections) to DB
// ---------------------------------------------------------------
async function regenerateTimetable() {
  ...
  try {
    ...
    // cache full timetable
    localTimetables = json.timetable;
    currentTimetableData = json.timetable;

    displayGeneratedTimetable(json.timetable);
    alert("Regenerated and saved as active version.");
  } catch (err) {
    console.error(err);
...
async function loadTimetableForCurrentSection() {
  ...
  // 1) If we have a locally generated timetable, use it
  if (localTimetables && section) {
    // localTimetables has ALL sections; displayGeneratedTimetable
    // already picks the current one by name.
    displayGeneratedTimetable(localTimetables);
    return;
  }

  // 2) Otherwise, load from saved timetable in DB
  try {
    const r = await fetch(`${API_URL}/api/timetable/load/${currentSectionId}`);
    const json = await r.json();
    ...
    // if no saved timetable, just leave grid showing only fixed/break cells
  } catch (e) {
